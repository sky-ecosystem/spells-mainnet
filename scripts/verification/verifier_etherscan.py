...
class VerifierEtherscan:
    ...
    def verify_contract(
        ...
        if library_address:
            cmd.extend(
                ["--libraries", f"src/DssExecLib.sol:DssExecLib:{library_address}"]
            )
        ...
        try:
            ...
            print(
                f"✓ Contract verified successfully at {self.get_verification_url(contract_address)}"
            )
            return True
            ...
            if (
                "already verified" in e.stderr.lower()
                or "already verified" in e.stdout.lower()
            ):
