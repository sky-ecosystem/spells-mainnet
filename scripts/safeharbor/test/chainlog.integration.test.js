...
test("loads the Safe Harbor agreement address from the Chainlog", async () => {
    ...
    const call = vi
        ...
        .mockResolvedValue("0x0000000000000000000000007000000000000000000000000000000000000001");

    try {
        const getChainlogAddress = createChainlogReader(provider);
        expect(call).not.toHaveBeenCalled();
        expect(await getChainlogAddress("SAFE_HARBOR_AGREEMENT")).toBe("0x7000000000000000000000000000000000000001");
