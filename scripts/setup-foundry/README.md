...
[Foundry's standard installation procedure](https://getfoundry.sh/getting-started/installation) bootstraps `foundryup` by piping a remotely hosted script directly into a shell:
...
This tool provides one auditable process for selecting, installing, and verifying Foundry. The selector chooses the newest release that:
...
Before reporting its source commit and setup-tool SHA-256, the tool confirms that the executed runtime Python files have the same paths and bytes as those files in the reported commit. A locally modified or untracked runtime file is rejected; tests, mocks, bytecode caches, and unrelated working-tree changes do not affect the reported digest.
...
| macOS on Apple Silicon | `arm64`, `aarch64`, or an `x86_64` process under Rosetta | `darwin_arm64` |

Native Windows, other operating systems, and other architectures are not supported.

## Prerequisites

Run the tool from this Git checkout with:

- Python 3.10 or newer;
- `gh`, authenticated to `github.com`; and
- `git`.

Display the built-in command reference without performing environment or network checks:

```bash
python3 ./scripts/setup-foundry/setup-foundry.py --help
```
...
Keep `FOUNDRY_IGNORE_AGE` set to `"0"` to enforce the 14-day cooling period. Set it to `"1"` only when the pinned release has an approved cooling-period waiver. Any other value fails before the setup tool runs.
