"""Render copyable evidence for Foundry installation and verification."""
...
def report_selection(selection, source_commit, tooling_hash):
    print(f"Desired Foundry release: {selection['version']}")
    ...
    if selection.get("archive_preflight_status"):
        print(f"Archive preflight: {selection['archive_preflight_status']}")
    print(f"spells-mainnet commit: {source_commit}")
    print(f"Setup CLI SHA-256: {tooling_hash}")


def report_verification_summary(
    selection, source_commit, tooling_hash, installed_tag, version_status
):
    print("\nEvidence summary:")
    print(f"  Source: spells-mainnet {source_commit}; setup CLI SHA-256 {tooling_hash}")
    print(
        f"  Desired release: {selection['version']}; {selection['published_at']}; {selection['release_url']}"
    )
    print(f"  Policy decision: {selection['selection_reason']}")
    print(f"  Installed release: {installed_tag}; {version_status}")
    print(
        f"  Binary attestations: forge, cast, anvil, and chisel verified against {SIGNER_WORKFLOW}"
    )
...
def report_installation_summary(selection, source_commit, tooling_hash):
    ...
    print(f"  Source: spells-mainnet {source_commit}; setup CLI SHA-256 {tooling_hash}")
    print(
        f"  Release: {selection['version']}; {selection['published_at']}; {selection['release_url']}"
    )
    print(f"  Policy decision: {selection['selection_reason']}")
...
def report_installation_path_status(destination):
    if str(destination) not in os.environ.get("PATH", "").split(os.pathsep):
        ...
        print("Required action: update-path", file=sys.stderr)
